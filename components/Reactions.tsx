...
const Reactions: FC<Props> = ({ postId }) => {
  ...
  // Methods
  const fillReactions = (newReactions: IReaction[]): IReaction[] => {
    return Object.keys(emojiLookup)
      .map((key) => {
        const reactionFound = newReactions.find(
  ...
  const getReactions = useCallback(async () => {
    if (reactId) {
      ...
      try {
        ...
        setReactions(fillReactions(reactionsRes.counts));
        setUserReactedOn(reactionsRes.user ? reactionsRes.user.emoji_id : null);
      } catch (e) {
        return;
      } finally {
        setReactionsLoading(false);
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
        {reactionsLoading
          ? new Array(6).fill(0).map((_, i) => (
              <div
                className="flex flex-col items-center gap-1 px-1 sm:gap-2"
                key={`${i}-reaction-skeleton`}
              >
